import re #for regex-based tokenization and punctuation removal
    ...
    def stem(self, word: str) -> str:
        ...
        for suffix, replacement in self._suffixes:
            if w.endswith(suffix) and len(w) - len(suffix) >= 2:
                return w[:-len(suffix)] + replacement # only replace if it leaves at least 2 chars
        return w
...
def preprocess( text: str, use_stemming: bool = True, use_lemmatization: bool = False, show_steps: bool = False) -> list[str]:

    steps = {}

    steps['original'] = text[:400] + ('...' if len(text) > 400 else '')

    lowered = case_fold(text)
    steps['case_folded'] = lowered[:400]

    no_punct = remove_punctuation(lowered)
    steps['no_punctuation'] = no_punct[:400]

    tokens = tokenize(no_punct)
    steps['tokenized'] = tokens[:50]

    no_stop = remove_stopwords(tokens)
    steps['stopwords_removed'] = no_stop[:50]

    filtered = filter_short_tokens(no_stop)

    if use_lemmatization:
        final_tokens = lemmatize_tokens(filtered)
        steps['lemmatized'] = final_tokens[:50]
    elif use_stemming:
        final_tokens = stem_tokens(filtered)
        steps['stemmed'] = final_tokens[:50]
    else:
        final_tokens = filtered

    steps['final_tokens'] = final_tokens[:50]
...
def preprocess_all( documents: dict[str, str], use_stemming: bool = True,use_lemmatization: bool = False) -> dict[str, list[str]]:
    return {
...
#  Comparison helper (stemming vs lemmatization)
...
# comparison table showing original token, stemmed version, lemmatized version, and whether they differ (stem != lemma)
    comparison = []
    for orig, stem, lemma in zip(filtered, stemmed, lemmatized):
        #zip function to iterate over original, stemmed, and lemmatized tokens in parallel
        comparison.append({
            'original': orig,
            'stemmed': stem,
            'lemmatized': lemma,
