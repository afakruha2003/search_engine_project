...
with st.sidebar:
    ...
    if st.session_state['docs_loaded']:
        st.markdown(f"<div class='index-built'>✓ {len(st.session_state['documents'])} docs loaded</div>", unsafe_allow_html=True)
        if st.session_state['indexes_built']:
            st.markdown("<div class='index-built'>✓ All indexes ready</div>", unsafe_allow_html=True)
...
def require_indexes():
    if not st.session_state['indexes_built']:
        ...
        <div class="warning-box">
            ⚠️ Please load documents and build indexes first using the sidebar button.
        </div>
        """, unsafe_allow_html=True)
        st.stop()
...
if page == "🏠 Home":
    ...
    for col, val, label in zip(
        [col1, col2, col3, col4],
        [n_docs, vocab, n_idx, "✓" if st.session_state['indexes_built'] else "✗"],
        ["Documents", "Vocabulary Size", "Indexes Built", "Status"]
    ):
        col.markdown(f"""
...
# ─────────────────────────────────────────────
    ...
    if doc_choice:
        ...
        step_labels = {
            ...
            'no_punctuation': '✂️ After Punctuation Removal',
        ...
        for entry in comparison['comparison_table'][:30]:
            rows.append({
                ...
                'Different?': '⚠️ Yes' if entry['different'] else '✓ Same'
            })
        st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
...
# ─────────────────────────────────────────────
    ...
    st.markdown("### Sample Matrix (first 20 terms × all docs)")
    sample = im.get_sample(n_terms=20)
    st.dataframe(sample, use_container_width=True)

    st.markdown("### ⚠️ Limitations")
    st.info(im.limitations())
...
# ─────────────────────────────────────────────
    ...
    st.markdown("### Sample Posting Lists (first 20 terms)")
    entries = ii.get_sample_entries(20)
...
# ─────────────────────────────────────────────
    ...
    st.markdown("### Sample Positional Entries (first 15 terms)")
    entries = pi.get_sample_entries(15)
    st.dataframe(pd.DataFrame(entries), use_container_width=True, hide_index=True)
            ...
            for r in results:
                ...
                <div class="result-card">
                    ...
                    <div class="score-bar-container">
                        <div style="background: #e94560; height: 6px; width: {bar_width}%; border-radius: 4px;"></div>
                    </div>
...
# ─────────────────────────────────────────────
    ...
    if col1.button("machine AND learning"):
        st.session_state['bool_example'] = "machine AND learning"
    if col2.button("data OR mining"):
        st.session_state['bool_example'] = "data OR mining"
    if col3.button("NOT python"):
        st.session_state['bool_example'] = "NOT python"

    if 'bool_example' in st.session_state and not bool_query:
        bool_query = st.session_state['bool_example']
    ...
    with tab1:
        ...
        if st.button("Search Wildcard") and wildcard_input:
            ...
            if matches:
                ...
                # Now retrieve documents containing any of these terms
                all_docs = set()
                for term in matches:
    ...
    with tab3:
        ...
        if st.button("Expand Query") and expand_input:
            ...
            with col2:
                ...
                for t in new_tokens:
                    st.markdown(f"<span class='tag' style='background:#2a4a0a;color:#7fff7f'>{t}</span>", unsafe_allow_html=True)

            # Run ranking with expanded query
...
st.markdown("---")
st.markdown(
    "<center><small>Mini Search Engine — Information Management Systems (SENG328)</small></center>",
    unsafe_allow_html=True
)
